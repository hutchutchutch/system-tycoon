...
- `npm run check`: zero lint warnings; 32 tests pass; frontend production build, Worker TypeScript, and generated binding checks pass.
...
- `git diff --check` passes. Existing unrelated worktree changes were preserved; nothing was staged or committed by this implementation.

## Release steps and remaining limitations

1. Back up production D1 and apply pending migrations through `0017_gameplay_integrity.sql` before releasing this frontend/Worker. Do not edit an already-applied migration; use a new migration for subsequent changes.
